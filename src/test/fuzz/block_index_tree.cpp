...
FUZZ_TARGET(block_index_tree, .init = initialize_block_index_tree)
{
        ...
        CallOneOf(
            ...
            [&] {
                ...
                if (!(prev_block->nStatus & BLOCK_FAILED_MASK)) {
                    ...
                    if (debug) std::cout << "MZ inserted:" << index->GetBlockHash() << "|height" << index->nHeight << "| size: " << blockman.m_block_index.size() << "|prev: " << index->pprev->GetBlockHash() << std::endl;
                }
            },
            [&] {
                ...
                if (index->nTx == 0 && !(index->nStatus & BLOCK_FAILED_MASK)) {
                    if (fuzzed_data_provider.ConsumeBool()) { // Invalid
                        ...
                        if (debug) std::cout << "MZ Rec Tx invalid:" << index->GetBlockHash() << "|" << index->nHeight << "|nTx:" << index->nTx << "| nChainTx:" << index->m_chain_tx_count << std::endl;
                    } else {
                        ...
                        if (debug) std::cout << "MZ Rec Tx after:" << index->GetBlockHash() << "|" << index->nHeight << "|nTx:" << index->nTx << "| nChainTx:" << index->m_chain_tx_count << std::endl;
                    }
                }
            },
            [&] {
                ...
                do {
                    ...
                    if (debug) std::cout << "MZ try to go from " << chain.Tip()->nHeight << "|" << chain.Tip()->GetBlockHash().ToString() << " to" << best_tip->nHeight << "|" << best_tip->GetBlockHash().ToString() << std::endl;
                    ...
                    for (CBlockIndex* block : to_connect | std::views::reverse) {
                        ...
                        if (!block->IsValid(BLOCK_VALID_SCRIPTS)) {
                            if (fuzzed_data_provider.ConsumeBool()) { // Invalid
                                ...
                                if (debug) std::cout << "MZ found invalid: " << block->nHeight << "|" << block->GetBlockHash().ToString() << std::endl;
                                break;
                            } else {
                ...
                if (debug) std::cout << "MZ ABC old tip h=" << old_tip->nHeight << "| new h=" << chain.Tip()->nHeight << "|" << chain.Tip()->GetBlockHash().ToString() << std::endl;
            ...
            [&] {
                ...
                if (prune_block != chain.Tip()) {
                    ...
                    if (debug) std::cout << "MZ pruned a block: height " << prune_height << " |hash:" << prune_block->GetBlockHash().ToString() << std::endl;
                }
            });
    }
    ...
    {
        if (debug) std::cout << "MZ Cleanup" << std::endl;
        LOCK(cs_main);
