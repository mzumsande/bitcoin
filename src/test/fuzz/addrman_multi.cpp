...
class AddrManMultiDeterministic : public AddrManMulti
{
public:
    explicit AddrManMultiDeterministic(std::vector<bool> asmap, uint256 seed)
        : AddrManMulti(std::move(asmap), /* deterministic */ true, /* consistency_check_ratio */ 0)
    {
...
void CompareAddrManImpls(AddrManDeterministic& addrman, AddrManMultiDeterministic& addrman_multi, std::vector<CAddress>& addresses)
{
...
FUZZ_TARGET_INIT(addrman_compare, initialize_multi_addrman)
{
    ...
    auto addr_man_ptr = std::make_unique<AddrManMultiDeterministic>(asmap, seed);
    AddrManDeterministic& addr_man = *addr_man_multi_ptr;
    AddrManMultiDeterministic& addr_man_multi = *addr_man_ptr;
    ...
    const AddrManMulti& const_addr_man_multi{addr_man_multi};
    ...
    // GetAddr can yield different results, because the order in vRandom is not preserved
    // This is due to a difference in AddSingle: The old addrman first adds to vRandom, and then
