    ...
    if (search_tried) {
        ...
        while (1) {
            ...
            for (i = 0; i < ADDRMAN_BUCKET_SIZE; ++i) {
                ...
                int node_id = LookupAddrmanEntry(search_tried, nKBucket, position);
                if (node_id != -1) break;
            }
            ...
            int position = (nKBucketPos + i) % ADDRMAN_BUCKET_SIZE;
            int nId = LookupAddrmanEntry(search_tried, nKBucket, position);
        ...
        while (1) {
            ...
            for (i = 0; i < ADDRMAN_BUCKET_SIZE; ++i) {
                ...
                int node_id = LookupAddrmanEntry(search_tried, nUBucket, position);
                if (node_id != -1) break;
            }
            // If the bucket is entirely empty, start over with a (likely) different one.
            if (i == ADDRMAN_BUCKET_SIZE) continue;
            // Find the entry to return.
            int position = (nUBucketPos + i) % ADDRMAN_BUCKET_SIZE;
            int nId = LookupAddrmanEntry(search_tried, nUBucket, position);
            const auto it_found{mapInfo.find(nId)};
            assert(it_found != mapInfo.end());
            const AddrInfo& info{it_found->second};
            // With probability GetChance() * fChanceFactor, return the entry.
