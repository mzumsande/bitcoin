...
void ChainstateManager::CheckBlockIndex()
{
    ...
    while (pindex != nullptr) {
        ...
        // All parents having had data (at some point) is equivalent to all parents being VALID_TRANSACTIONS
        // This is equivalent to HaveNumChainTxs(), except in a special case where we receive data for a direct child of a pruned block, in which case
        // HaveNumChainTxs may not be set. This can only happen if we never connected the child, so guard the next two checks by pindexFirstNotScriptsValid
        // HaveNumChainTxs will also be set in the assumeutxo snapshot block from snapshot metadata.
        if (!pindexFirstNotScriptsValid) {
        ...
        if (!pindex->pprev) {
            ...
            if (!pindexFirstNotScriptsValid) assert(pindex->m_chain_tx_count == pindex->nTx + pindex->pprev->m_chain_tx_count);
        } else {
