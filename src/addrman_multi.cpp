...
void AddrManImpl::Serialize(Stream& s_) const
{
     ...
     * deserialization.
     *
     * vvNew, vvTried, mapInfo, mapAddr and vAddrStatistics are never encoded explicitly;
    ...
    for (auto it = m_index.get<ByBucket>().begin(); it != m_index.get<ByBucket>().end();) {
        ...
        s << MakeAddress(*it);
        AddrStatistics& stats = vAddrStatistics[it->nRandomPos];
        s << stats.nLastTry;
...
void AddrManImpl::Unserialize(Stream& s_)
{
    ...
    for (int i = 0; i < read_new + read_tried; ++i) {
        ...
        if (format >= Format::V5_MULTIINDEX) {
            ...
            //TODO MZ: read in earlier format correctly!
            s >> info;
        }
...
void AddrManImpl::EraseInner(AddrManIndex::index<ByAddress>::type::iterator it)
{
    ...
    if (it->nRandomPos != -1) {
        ...
        if (it_alias != m_index.get<ByAddress>().end()) {
            ...
            SwapRandom(it->nRandomPos, vAddrStatistics.size() - 1);
            vAddrStatistics.pop_back();
        }
    }
...
void AddrManImpl::SwapRandom(unsigned int nRndPos1, unsigned int nRndPos2) const
{
    ...
    assert(nRndPos1 < vAddrStatistics.size() && nRndPos2 < vAddrStatistics.size());

    auto it1 = vAddrStatistics[nRndPos1];
    auto it2 = vAddrStatistics[nRndPos2];

    it1.addr->nRandomPos = nRndPos2;
    it2.addr->nRandomPos = nRndPos1;

    vAddrStatistics[nRndPos1] = it2;
...
bool AddrManImpl::AddSingle(const CAddress& addr, const CNetAddr& source, int64_t nTimePenalty)
{
    ...
    if (it != m_index.get<ByAddress>().end()) {
        ...
        AddrStatistics& stat = vAddrStatistics[it->nRandomPos];
...
void AddrManImpl::Attempt_(const CService& addr, bool fCountFailure, int64_t nTime)
{
    ...
    // update info
    auto last_good = nLastGood;

    AddrStatistics& addrStats = vAddrStatistics[it->nRandomPos];
    addrStats.nLastTry = nTime;
    if (fCountFailure && addrStats.nLastCountAttempt < last_good) {
    ...
    if (!newOnly &&
       (nTried > 0 && (nNew == 0 || insecure_rand.randbool() == 0))) {
        ...
        while (1) {
            ...
            // With probability GetChance() * fChanceFactor, return the entry.
            AddrStatistics& stats{vAddrStatistics[it_canonical->nRandomPos]};
            if (insecure_rand.randbits(30) < fChanceFactor * GetChance(stats) * (1 << 30)) {
...
std::vector<CAddress> AddrManImpl::GetAddr_(size_t max_addresses, size_t max_pct, std::optional<Network> network) const
{
    ...
    size_t nNodes = vAddrStatistics.size();
    if (max_pct != 0) {
    ...
    for (unsigned int n = 0; n < vAddrStatistics.size(); n++) {
        if (addresses.size() >= nNodes)
            break;

        int nRndPos = insecure_rand.randrange(vAddrStatistics.size() - n) + n;
        ...
        // Filter for quality
        if (IsTerrible(vAddrStatistics[n], now)) continue;
...
std::pair<CAddress, int64_t> AddrManImpl::SelectTriedCollision_()
{
    ...
    if (it_old != m_index.get<ByBucket>().end()) return {MakeAddress(*it_old), vAddrStatistics[it_old->nRandomPos].nLastTry};
    return {};
}
...
int AddrManImpl::CheckAddrman() const
{
    ...
    LOG_TIME_MILLIS_WITH_CATEGORY_MSG_ONCE(
        strprintf("new %i, tried %i, total %u", nNew, nTried, vAddrStatistics.size()), BCLog::ADDRMAN);
    ...
    for (auto it = m_index.get<ByAddress>().begin(); it != m_index.get<ByAddress>().end(); ++it) {
        ...
        if (info.nRandomPos == -1) {
            ...
            if ((size_t) info.nRandomPos >= vAddrStatistics.size()) return -22;
            if (vAddrStatistics[info.nRandomPos].addr != it) return -23;
            if (info.fInTried) {
                counted_tried++;
            } else {
                counted_new++;
    ...
    if ((size_t) (counted_new + counted_tried) != vAddrStatistics.size()) return -8;
